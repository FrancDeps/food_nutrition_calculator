...
for food, info in st.session_state.daily_data.items():
    if food in food_database:
        quantity = info["quantity"] / 100  # Convert quantity to per 100g
        for macro in macronutrient_totals:
            macronutrient_totals[macro] += food_database[food].get(macro, 0) * quantity

# **Debugging Output**
st.write("🔍 Macronutrient Totals:", macronutrient_totals)
...
if total_macros > 0:
    ...
    # **Debugging Output**
    st.write("📊 Macronutrient Percentages:", macronutrient_percentages)
